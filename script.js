...
class Component {
  ...
  // ==================== MODÈLE DE STOCK : EMPREINTE, CONTRÔLE, CALCULS ====================
  // Le classeur de stock est la pièce la plus fragile du dispositif : ses onglets sont truffés de
  // formules qui se référencent d'une feuille à l'autre par coordonnées fixes. Supprimer une
  // colonne suffit à transformer le récapitulatif des ventes en #REF!, et une espèce sans vente
  // suffit à le transformer en #VALUE! (le prix moyen renvoie le texte « -  € / Kg », que le
  // récapitulatif multiplie ensuite par un poids). Les deux cas se produisent dans les fichiers
  // réels : sur les semaines 27 et 31, TOTAL VENTES est illisible alors que les données sont là.
  //
  // D'où ce circuit. Le tableau de bord retient une EMPREINTE du fichier modèle désigné par
  // l'utilisatrice, relit chaque fichier de semaine avec une détection TOLÉRANTE (les libellés
  // varient : « COMMANDES - SORTIE » ici, « VENTE - SORTIE » là, « ACHAT -  ENTREE » avec deux
  // espaces ailleurs), compare les deux, et recalcule lui-même tous les totaux. Plus aucune
  // formule n'est nécessaire pour que les chiffres soient justes.
  static STOCKMOD_KEY = 'avStockModele';
  // Marqueurs de structure, cherchés dans les 5 premières colonnes après normalisation.
  static STOCK_MARQUEURS = [
    { cle: 'stockPrecedent', re: /^stock precedent/ },
    { cle: 'achatDebut', re: /^achat *- *entree/ },
    { cle: 'sortieDebut', re: /^(commandes?|vente) *- *sortie/ },
    { cle: 'achatTotal', re: /^total achat/ },
    { cle: 'sortieTotal', re: /^total (commande|vente)/ },
    { cle: 'resume', re: /^resumee? benefice/ },
    { cle: 'prixAchat', re: /^stock *\/ *prix achat/ },
    { cle: 'prixVendu', re: /^stock *\/ *prix +vendu/ },
    { cle: 'benefices', re: /^benefices?$/ },
    { cle: 'stockActuel', re: /^stock actuel/ },
  ];
  _colNum(lettre) {
    let n = 0; const s = String(lettre || '').toUpperCase();
    for (let i = 0; i < s.length; i++) { const c = s.charCodeAt(i) - 64; if (c < 1 || c > 26) return -1; n = n * 26 + c; }
    return n - 1;
  }
  // Lit la géométrie d'une feuille d'espèce SANS rien présumer de ses coordonnées : seuls les
  // libellés font foi. C'est ce qui permet au contrôle de fonctionner sur un fichier déjà décalé.
  _stockLireStructure(rows) {
    rows = rows || [];
    // String() OBLIGATOIRE : _norm appelle toLowerCase, et une cellule de stock contient souvent
    // un NOMBRE (le stock précédent, un code client). Sans cette conversion, la lecture de
    // structure plante et le fichier entier devient illisible.
    const nrm = s => this._norm(s == null ? '' : String(s)).replace(/\s+/g, ' ').trim();
    const st = { tailles: [] };
    for (let r = 0; r < Math.min(rows.length, 120); r++) {
      for (let c = 0; c < 5; c++) {
        const t = nrm((rows[r] || [])[c]);
        if (!t) continue;
        if (t === 'clients' && st.entete == null) { st.entete = r; st.clients = c; }
        Component.STOCK_MARQUEURS.forEach(m => { if (st[m.cle] == null && m.re.test(t)) st[m.cle] = r; });
      }
    }
    if (st.entete == null) return null;
    // Colonnes : triplets (poids, prix unitaire, prix total) repérés par le « PRIX … / Kg » qui
    // suit toujours le libellé de calibre, puis les colonnes de synthèse en fin de tableau.
    const hdr = rows[st.entete] || [];
    for (let c = 0; c < hdr.length; c++) {
      const t = String(hdr[c] == null ? '' : hdr[c]).trim(); const T = nrm(t);
      if (!T) continue;
      if (/^poids total/.test(T)) { st.poidsTotal = c; continue; }
      if (/^prix moyen/.test(T)) { st.prixMoyen = c; continue; }
      if (/^prix total/.test(T) && st.poidsTotal != null && st.prixTotal == null) { st.prixTotal = c; continue; }
      if (T === 'clients' || /prix/.test(T)) continue;
      if (/^prix/.test(nrm(hdr[c + 1] == null ? '' : String(hdr[c + 1])))) { st.tailles.push({ t, p: c, u: c + 1, m: c + 2 }); c += 2; }
    }
    return st.tailles.length ? st : null;
  }
  // Empreinte complète d'un classeur de stock : l'ordre des onglets et la géométrie de chacun.
  _stockEmpreinte(wb, nomFichier) {
    const feuilles = {};
    (wb || []).forEach(sh => { const st = this._stockLireStructure(sh.rows); if (st) feuilles[sh.name] = st; });
    return { fichier: nomFichier || '', onglets: (wb || []).map(s => s.name), feuilles, ts: Date.now() };
  }
  modeleStock() { try { return JSON.parse(localStorage.getItem(Component.STOCKMOD_KEY) || 'null'); } catch (e) { return null; } }
  // Le modèle ne se modifie QUE par l'import d'un nouveau fichier : c'est lui la référence, il ne
  // doit jamais dériver au gré des fichiers de semaine qu'on ouvre.
  enregistrerModeleStock(wb, nomFichier) {
    const emp = this._stockEmpreinte(wb, nomFichier);
    const n = Object.keys(emp.feuilles).length;
    if (!n) { this.setState({ msg: { kind: 'error', text: `« ${nomFichier} » ne ressemble pas à un classeur de stock : aucune feuille d'espèce reconnue (ligne « CLIENTS » et calibres introuvables).` } }); return null; }
    this.saveJSON(Component.STOCKMOD_KEY, emp);
    this.setState({ stockModTick: (this.state.stockModTick || 0) + 1, msg: { kind: 'ok', text: `Modèle de stock enregistré depuis « ${nomFichier} » : ${n} feuille${n > 1 ? 's' : ''} d'espèce, ${emp.onglets.length} onglets. C'est désormais cette structure qui fait référence.` } });
    return emp;
  }
  // Compare un fichier de semaine à l'empreinte du modèle. Ne modifie RIEN : le décalage de
  // colonnes est signalé sans être corrigé, parce que le remettre en place déplacerait les
  // données saisies depuis — perdre une semaine de saisie serait pire que le décalage lui-même.
  controleStructureStock(wb, nomFichier) {
    const ref = this.modeleStock();
    if (!ref) return { sansModele: true, ecarts: [] };
    const ecarts = [];
    const noms = (wb || []).map(s => s.name);
    ref.onglets.forEach(o => { if (!noms.includes(o)) ecarts.push({ feuille: o, grave: true, texte: `onglet absent du fichier` }); });
    noms.forEach(o => { if (!ref.onglets.includes(o)) ecarts.push({ feuille: o, grave: false, texte: `onglet en plus, absent du modèle` }); });
    Object.keys(ref.feuilles).forEach(nom => {
      const sh = (wb || []).find(s => s.name === nom); if (!sh) return;
      const cur = this._stockLireStructure(sh.rows);
      const r = ref.feuilles[nom];
      if (!cur) { ecarts.push({ feuille: nom, grave: true, texte: 'structure méconnaissable : ni ligne « CLIENTS » ni calibres' }); return; }
      ['entete', 'stockPrecedent', 'achatDebut', 'achatTotal', 'sortieDebut', 'sortieTotal', 'prixAchat', 'prixVendu', 'benefices', 'stockActuel'].forEach(k => {
        if (r[k] == null && cur[k] == null) return;
        if (cur[k] == null) { ecarts.push({ feuille: nom, grave: true, cle: k, texte: `repère « ${k} » introuvable (attendu ligne ${r[k] + 1})` }); return; }
        if (r[k] != null && r[k] !== cur[k]) ecarts.push({ feuille: nom, grave: false, cle: k, texte: `« ${k} » ligne ${r[k] + 1} → ${cur[k] + 1}` });
      });
      const rt = r.tailles.map(x => x.t).join('|'), ct = cur.tailles.map(x => x.t).join('|');
      if (rt !== ct) ecarts.push({ feuille: nom, grave: true, texte: `calibres différents : « ${rt} » → « ${ct} »` });
      else r.tailles.forEach((x, i) => { if (x.p !== cur.tailles[i].p) ecarts.push({ feuille: nom, grave: true, texte: `calibre « ${x.t} » déplacé de la colonne ${this._colLetter(x.p + 1)} à ${this._colLetter(cur.tailles[i].p + 1)} — colonne supprimée ou insérée` }); });
    });
    return { sansModele: false, fichier: nomFichier || '', ecarts, ok: !ecarts.length };
  }
  // Recalcule TOUT ce qui était une formule, à partir des seules cellules saisies à la main
  // (poids et prix unitaire). `annulees` est l'ensemble des lignes barrées : elles sont exclues
  // des totaux, ce qui n'était pas possible tant que les totaux venaient d'Excel.
  calculsStock(wb, annulees) {
    const skip = annulees instanceof Set ? annulees : new Set();
    const num = v => { if (v == null || v === '') return 0; const n = typeof v === 'number' ? v : parseFloat(String(v).replace(/\s/g, '').replace(',', '.')); return isFinite(n) ? n : 0; };
    const out = { feuilles: [], recap: [], totalVentes: 0, nonCalculables: [] };
    (wb || []).forEach(sh => {
      const st = this._stockLireStructure(sh.rows); if (!st) return;
      const rows = sh.rows || [];
      const nrmC = v => this._norm(v == null ? '' : String(v)).replace(/\s+/g, ' ').trim();
      // Structure d'une AUTRE génération (pas de bloc achat/sortie identifiable) : on ne calcule
      // PAS. Renvoyer 0 en silence ferait croire à une semaine sans activité alors que les
      // données sont là — c'est exactement ce qui arrive sur les anciens fichiers (semaine 27).
      if (st.achatDebut == null || st.sortieDebut == null) {
        out.nonCalculables.push({ nom: sh.name, raison: st.achatDebut == null ? 'bloc « ACHAT - ENTREE » introuvable' : 'bloc « VENTE / COMMANDES - SORTIE » introuvable' });
        return;
      }
      // Une ligne de TOTAL porte les mêmes colonnes que les lignes de données : l'inclure dans la
      // somme double tous les chiffres. Sur la semaine 31, dont la feuille HOMARD a perdu son
      // libellé « TOTAL COMMANDE SORTIE », le bloc courait jusqu'au résumé et avalait les lignes
      // de total : le prix moyen du homard ressortait à 743 €/kg et les ventes à 1,2 M€.
      const estTotal = r => {
        const c = st.clients != null ? st.clients : 2;
        for (let k = Math.max(0, c - 2); k <= c + 1; k++) {
          if (/^(total|sous.?total|cumul|report)/.test(nrmC((rows[r] || [])[k]))) return true;
        }
        return false;
      };
      const bloc = (debut, fin) => {
        const res = { tailles: st.tailles.map(() => ({ poids: 0, montant: 0 })), poids: 0, montant: 0, moyen: 0, lignes: 0 };
        if (debut == null || fin == null || fin <= debut) return res;
        for (let r = debut; r < fin; r++) {
          if (estTotal(r)) break;
          if (skip.has(`${sh.name}|${r}`)) continue;
          let vide = true;
          st.tailles.forEach((t, i) => {
            const p = num((rows[r] || [])[t.p]), u = num((rows[r] || [])[t.u]);
            if (!p && !u) return;
            vide = false;
            res.tailles[i].poids += p; res.tailles[i].montant += p * u;
            res.poids += p; res.montant += p * u;
          });
          if (!vide) res.lignes++;
        }
        res.moyen = res.poids > 0 ? res.montant / res.poids : 0;
        return res;
      };
      const achat = bloc(st.achatDebut, st.achatTotal != null ? st.achatTotal : st.sortieDebut);
      const sortie = bloc(st.sortieDebut, st.sortieTotal != null ? st.sortieTotal : st.resume != null ? st.resume : st.prixAchat);
      const f = {
        nom: sh.name, achat, sortie,
        benefice: sortie.montant - achat.montant,
        stockActuel: achat.poids - sortie.poids,
        stockPrecedent: st.stockPrecedent != null ? num((rows[st.stockPrecedent] || [])[st.clients != null ? st.clients : 2]) : 0,
        structure: st,
      };
      out.feuilles.push(f);
      // Le récapitulatif reprend TOUTES les espèces : celui du modèle n'en comptait que 5 sur 9,
      // et 4 de ces 5 lignes visaient de mauvaises cellules.
      out.recap.push({ nom: sh.name, poids: sortie.poids, moyen: sortie.moyen, total: sortie.montant });
      out.totalVentes += sortie.montant;
    });
    return out;
  }
